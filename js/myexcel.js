...
(function() {
  ...
  $JExcel.toExcelUTCTime = function(date1) {
    ...
    var seconds =
      date1.getUTCSeconds() +
      60 * date1.getUTCMinutes() +
      60 * 60 * date1.getUTCHours(); // Number of seconds of received hour
  ...
  function getAsXml(sheet) {
    return templateSheet
      .replace("{views}", generateViews(sheet.views))
    ...
    var oSheets = {
      ...
      rows: function(i) {
        if (i < 0 || i >= this.sheets.length)
          throw "Bad sheet number must be [0.." +
      ...
      setWidth: function(sheet, column, value, style) {
        ...
        if (value)
          this.sheets[sheet].colWidths[column] = isNaN(value)
      ...
      if (vals[0] == "NONE") s = s + "/>";
        ...
        if (border)
          s =
            ...
            '" >' +
            (vals[1] != "NONE"
              ? '<color rgb="FF' + vals[1].substring(1) + '"/>'
              : "");
        else s = s + ">";
        s = s + "</" + borderKind[i] + ">";
      }
  ...
  function normalizeFont(fontDescription) {
    ...
    var i = 0,
      list = fontDescription.split(" "); //  Split by " "
    var name = [];
    while (
      list[0] &&
      list[0] != "none" &&
      ...
      name.push(
        list[0].charAt(0).toUpperCase() + list[0].substring(1).toLowerCase()
      );
      list.splice(0, 1);
    }
  ...
  function normalizeAlign(a) {
    if (!a) return "---";
    var a = replaceAllMultiple(a.toString() + " - - -", "  ", " ")
      .trim()
      .toUpperCase()
      .split(" ");
  ...
  function normalizeBorders(b) {
    ...
    for (var i = 0; i < 4; i++) {
      ...
      l[i] = (
        (l[i].substring(0, 4) == "NONE" ? "NONE" : l[i]).trim() +
        " NONE NONE NONE"
      ).trim();
      var st = l[i].split(" ");
      if (st[0].charAt(0) == "#") {
  ...
  function createStyleSheet(defaultFont) {
    ...
    var oStyles = {
      add: function(a) {
        ...
        if (a.font)
          style.font = findOrAdd(
            fonts,
            normalizeFont(a.font.toString().trim())
          );
        ...
        if (a.border)
          style.border =
            1 +
            findOrAdd(borders, normalizeBorders(a.border.toString().trim())); // There is a HARDCODED border
    ...
    oStyles.toStyleSheet = function() {
      ...
      for (var i = baseFormats; i < formats.length; i++)
        s =
          s + '<numFmt numFmtId="' + i + '" formatCode="' + formats[i] + '"/>';
      s = s + "</numFmts>";
  ...
  function generateCell(cell, column, row, mergeCells) {
    if (cell.colspan > 1) {
      var m = {
        from: cellName(column, row),
        to: cellName(column + cell.colspan - 1, row)
      };
      mergeCells.push(m);
    }
    ...
    if (cell.isstring || isNaN(value)) {
      if (cell.isstring || value.charAt(0) != "=")
  ...
  function generateRow(row, index, mergeCells) {
    ...
    for (var i = 0; i < row.cells.length; i++) {
      if (row.cells[i])
        oCells.push(generateCell(row.cells[i], i, rowIndex, mergeCells));
    }
  ...
  function generateColums(columns) {
    ...
    for (var i = 0; i < columns.length; i++) {
      ...
      if (c) {
        ...
        if (c.wt == "auto") s = s + ' width="18" bestFit="1" customWidth="1" ';
  ...
  $JExcel.new = function(defaultFont) {
    ...
    excel.set = function(s, column, row, value, style, colspan) {
      if (isObject(s))
        return this.set(s.sheet, s.column, s.row, s.value, s.style); // If using Object form, expand it
      ...
      if (!isNaN(column)) {
        ...
        if (!isNaN(row)) {
          ...
          return setCell(
            s.getCell(column, row),
            value,
            style,
            isstring,
            colspan
          ); // and also a ROW operation the this is a CELL operation
        }
    ...
    excel.generate = function(filename) {
      ...
      zip
        .generateAsync({ type: "blob", mimeType: "application/vnd.ms-excel" })
